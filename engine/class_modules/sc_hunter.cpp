...
struct kill_shot_t : hunter_ranged_attack_t
{
  kill_shot_t( hunter_t* p, const std::string& options_str ):
    hunter_ranged_attack_t( "kill_shot", p, p -> spells.kill_shot )
  {
    parse_options( options_str );
  }
  ...
  {
    return candidate_target -> health_percentage() <= data().effectN( 2 ).base_value() &&
...
void hunter_t::init_spells()
{
  ...
  spells.kill_shot            = find_specialization_spell( "Kill Shot" );
